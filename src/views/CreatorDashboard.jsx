...
function PageCommunity({ T, dark, setDark }) {
  ...
  const [badge, setBadge] = useState('Verified PR Author');
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      setUsers(res.data?.contributors || res.data?.users || []);
    } catch (err) {
  ...
  const handleFeature = async (user) => {
    setSaving(user.id);
    try {
      await api.post('/admin/contributors/feature', {
        user_id: user.id,
        role_title: roleTitle || user.role || 'Verified Notes Contributor',
        badge: badge || 'Verified PR Author',
      });
      await fetchUsers();
      setEditingUser(null);
      setRoleTitle('');
      setBadge('Verified PR Author');
    } catch (err) {
      alert('Failed to feature contributor: ' + (err.response?.data?.error || err.message));
    } finally {
      setSaving(null);
    }
  };

  const handleUnfeature = async (userId) => {
    setSaving(userId);
    try {
      await api.delete(`/admin/contributors/feature/${userId}`);
      await fetchUsers();
    } catch (err) {
      alert('Failed to unfeature contributor: ' + err.message);
    } finally {
      setSaving(null);
    }
  };
  ...
  useEffect(() => {
    if (showManage && users.length === 0) fetchUsers();
  }, [showManage]);

  const filtered = users.filter(u =>
    (u.name || '').toLowerCase().includes(searchQ.toLowerCase()) ||
    (u.username || '').toLowerCase().includes(searchQ.toLowerCase()) ||
    (u.college_name || '').toLowerCase().includes(searchQ.toLowerCase())
  );

  const featuredCount = users.filter(u => u.is_featured).length;

  return (
    <div className="page-enter">
      <PageHeader title="Community" sub={`15 followers · ${featuredCount} featured contributors`} dark={dark} setDark={setDark} T={T}/>

      {/* ── Manage Contributors Toggle Button ── */}
      <div style={{...cs, padding:"14px", marginBottom:10}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontSize:14,fontWeight:700,color:T.txt,display:"flex",alignItems:"center",gap:8}}>
              {React.cloneElement(Icons.users,{size:16,color:"#8A2BFF"})}
              Manage Contributors
            </div>
            <div style={{fontSize:11,color:T.txt3,marginTop:2}}>Add or remove users from the /contributors page</div>
          </div>
          <button onClick={()=>setShowManage(!showManage)} style={{
            fontSize:11,fontWeight:600,color:showManage?T.txt3:"#fff",
            padding:"6px 14px",borderRadius:7,
            border:showManage?`1px solid ${T.border}`:"none",
            background:showManage?T.card:`linear-gradient(90deg,#8A2BFF,#2D7FFF)`,
            cursor:"pointer",transition:"all 0.2s",
          }}>
            {showManage ? "Close" : "Open Manager"}
          </button>
        </div>
          ...
          <div style={{marginTop:16}}>
            ...
            <input
              ...
              style={{
                ...
                background:T.bg,border:`1px solid ${T.border}`,color:T.txt,
                fontSize:12,outline:"none",marginBottom:12,
              }}
            />

            {/* Featured Contributors Summary */}
            <div style={{
              display:"flex",gap:8,marginBottom:14,padding:"8px 12px",
              borderRadius:8,background:`#8A2BFF12`,border:`1px solid #8A2BFF30`,
            }}>
              <span style={{fontSize:11,color:"#8A2BFF",fontWeight:700}}>
            ...
            {loadingUsers ? (
              <div style={{textAlign:"center",padding:"24px 0",color:T.txt3,fontSize:12}}>Loading users...</div>
            ) : filtered.length === 0 ? (
              <div style={{textAlign:"center",padding:"24px 0",color:T.txt3,fontSize:12}}>No users found</div>
            ) : (
              <div style={{maxHeight:420,overflowY:"auto",display:"flex",flexDirection:"column",gap:6}}>
                {filtered.map(u => (
                  <div key={u.id} style={{
                    ...
                    background:u.is_featured?`${T.card}`:`${T.surf}`,
                    border:`1px solid ${u.is_featured?"#8A2BFF33":T.border}`,
                    transition:"all 0.15s",
                  }}>
                    {/* Avatar */}
                    <img
                      src={u.avatar_url || 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&q=80&w=80'}
                      alt={u.name || u.username}
                      style={{width:36,height:36,borderRadius:"50%",objectFit:"cover",border:u.is_featured?"2px solid #8A2BFF":`2px solid ${T.border}`,flexShrink:0}}
                    />

                    {/* User Info */}
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",gap:6}}>
                        <span style={{fontSize:12,fontWeight:600,color:T.txt,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{u.name || u.username}</span>
                        {u.is_featured && (
                          <span style={{fontSize:9,fontWeight:700,padding:"1px 6px",borderRadius:4,background:"#8A2BFF18",color:"#8A2BFF",border:"1px solid #8A2BFF33",flexShrink:0}}>
                            FEATURED
                          </span>
                        )}
                      </div>
                      <div style={{fontSize:10,color:T.txt3,marginTop:1}}>
                        @{u.username} · {u.college_name || 'No college'} · {u.posts_count || 0} posts
                      </div>
                    </div>
                    ...
                    <div style={{display:"flex",gap:6,flexShrink:0}}>
                      {u.is_featured ? (
                        <button
                          onClick={()=>handleUnfeature(u.id)}
                          disabled={saving===u.id}
                          style={{
                            fontSize:10,fontWeight:600,color:"#E8354A",
                            padding:"5px 10px",borderRadius:6,
                            border:"1px solid #E8354A33",background:"#E8354A12",
                        ...
                        <button
                          ...
                          disabled={saving===u.id}
                          style={{
                            fontSize:10,fontWeight:600,color:"#8A2BFF",
                            padding:"5px 10px",borderRadius:6,
                            border:"1px solid #8A2BFF33",background:"#8A2BFF12",
                            cursor:"pointer",opacity:saving===u.id?0.5:1,
                          }}
                        >
                          {saving===u.id?"...":(editingUser===u.id?"Confirm":"+ Feature")}
                        </button>
                      )}
                    </div>
                  </div>
                ...
                {editingUser && (
                  <div style={{
                    ...
                    background:`#8A2BFF08`,border:"1px solid #8A2BFF30",
                    marginTop:4,
                  }}>
                    <div style={{fontSize:11,fontWeight:700,color:"#8A2BFF",marginBottom:8}}>
                      Configure Featured Contributor
                    </div>
                    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                      <input
                        value={roleTitle}
                        onChange={e=>setRoleTitle(e.target.value)}
                        placeholder="Role title (e.g. Campus Lead & AI/ML Contributor)"
                        style={{
                          flex:1,minWidth:180,padding:"7px 10px",borderRadius:6,
                          background:T.bg,border:`1px solid ${T.border}`,color:T.txt,fontSize:11,outline:"none",
                        }}
                      />
                      <select
                        value={badge}
                        ...
                        style={{
                          ...
                          background:T.bg,border:`1px solid ${T.border}`,color:T.txt,fontSize:11,
                        }}
                      >
                        <option value="Top Reviewer">Top Reviewer</option>
                        <option value="Gold Contributor">Gold Contributor</option>
                        <option value="Verified PR Author">Verified PR Author</option>
                        <option value="Campus Lead">Campus Lead</option>
                        <option value="Subject Expert">Subject Expert</option>
                      </select>
                      <button
                        onClick={()=>{setEditingUser(null);setRoleTitle('');}}
                        style={{fontSize:10,color:T.txt3,padding:"5px 10px",borderRadius:6,border:`1px solid ${T.border}`,background:"transparent",cursor:"pointer"}}
                      >
                        Cancel
                      </button>
                    </div>
