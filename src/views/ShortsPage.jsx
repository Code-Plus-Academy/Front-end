...
function SideRail({ video, onLike, onSave, onShare, navigate }) {
  ...
  const actions = [
    { key: 'like', icon: (on) => (<svg width="27" height="27" viewBox="0 0 24 24" fill={on ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2"><path d="M7 22V11M2 13v7a2 2 0 002 2h13.4a2 2 0 001.96-1.6l1.44-6A2 2 0 0017.84 12H15V6a3 3 0 00-3-3 1 1 0 00-1 1v.5L7.5 11"/></svg>), label: fmtN(video.likes_count), on: video.viewer_liked, color: T.red, action: onLike },
    ...
    <>
      ...
      <div style={{ position: 'fixed', inset: 0, background: '#0b0f14', display: 'flex', justifyContent: 'center', alignItems: 'center', overflow: 'hidden', zIndex: 1 }}>
        ...
        <div style={{ position: 'relative', width: 'min(100vw, 450px)', maxWidth: '450px', height: '100dvh', background: '#000', overflow: 'hidden', boxShadow: '0 0 60px rgba(0,0,0,0.7)', borderRadius: 'clamp(0px, (100vw - 451px) * 999, 12px)' }}>
          ...
          <SideRail video={activeVideo} onLike={() => raw && handleLike(raw)} onSave={() => raw && handleSave(raw)} onShare={() => raw && handleShare(raw)} navigate={navigate} />
