...
function NotFoundScreen() {
  return (
    <div style={{ minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, textAlign: 'center' }}>
      ...
      <Link href="/feed"><button className="btn-secondary">Go to Feed</button></Link>
    </div>
  );
}
...
export function ActivityResolver() {
  ...
  const slug = pathname.replace(/^\/activity:/, '').split('/')[0];
...
export function DevProfile() {
  ...
  useRedirectEffect(() => { _router.replace(username ? `/u/${username}` : '/feed'); }, []);
  return null;
}
