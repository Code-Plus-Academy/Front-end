...
/** Get embed URL for any platform */
...
/** True if URL looks like a direct video file */
...
function CPACreatorCard({ video, t, user }) {
  ...
  return (
    <div style={{
      ...
      <div onClick={() => router.push(`/u/${video.creator_username}`)} style={{ cursor: 'pointer' }}>
        <Avatar src={video.creator_avatar} name={video.creator_name} size={46} />
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
          <span
            onClick={() => router.push(`/u/${video.creator_username}`)}
...
function OriginalCreatorCard({ video, t }) {
  ...
  // Only show if we have external creator info or a non-direct source
