  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '14px 24px', background: 'rgba(23,28,33,0.4)', backdropFilter: 'blur(16px)', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0, borderBottom: '1px solid rgba(74,68,87,0.15)' }}>
        ...
        {other && (
          <Link href={`/u/${other.username}`} style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, textDecoration: 'none' }}>
...
export function DMInbox() {
  ...
  const handleConvClick = (conv) => {
    if (isMobile) router.push(`/direct/${conv.id}`);
  ...
  return (
    <>
      ...
      <div style={{ height: 'calc(100vh - 104px)', display: 'flex', flexDirection: 'column', borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(74,68,87,0.2)', background: '#0f1419' }}>
        <ThreadPanel conversationId={conversationId} onBack={() => router.push('/messages')} />
      </div>
    </>
  );
}
